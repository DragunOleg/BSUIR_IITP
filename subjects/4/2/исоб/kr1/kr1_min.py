...
class TCP:
 def __init__(self,source_port,destination_port,ip):
  ...
  self.urgent=None
 def __str__(self):
  return 'Source {}:{}, Destination {}:{}, Seq: {}, Ack: {} Payload: "{}"'.format(self.ip.source_ip,self.source_port,self.ip.destination_ip,self.destination_port,self.sequence,self.acknowledgment,self.ip.payload)
class Connection:
